...
import Replica.VDOM       (Attr (ABool, AText), HTML,
                           VDOM (VLeaf, VNode, VRawText, VText), clientDriver)

indexWithBootstrap :: HTML
indexWithBootstrap =
...
customCss =
  VNode "style" (fl [("type", AText "text/css")]) Nothing
    [VText ".window { margin-right: auto; margin-left: auto; max-width: 960px;}"]
  where
    fl = fromList
